        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="teacher in filteredTeachers" :key="teacher.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="w-8 h-8 bg-indigo-100 dark:bg-indigo-900 rounded-full flex items-center justify-center mr-3">
                    <span class="text-indigo-600 dark:text-indigo-300 font-semibold">{{ teacher.name?.charAt(0) || 'T' }}</span>
                  </div>
                  <div>
              ...
              <td class="px-6 py-4">
                <div class="text-sm">
                  <p class="text-gray-600 dark:text-gray-300">{{ teacher.phone || 'N/A' }}</p>
                </div>
              </td>
              <td class="px-6 py-4 text-right">
                <button 
                  @click="removeClassTeacher(teacher.id)" 
          ...
          <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
            <div class="flex items-start">
              ...
              <p class="text-sm text-yellow-800 dark:text-yellow-300">
                Class teachers can mark daily attendance for their assigned class. Each class/section can have only one class teacher.
              </p>
            </div>
          </div>
...
const filteredTeachers = computed(() => {
  ...
  return classTeachers.value.filter(t => 
    t.name?.toLowerCase().includes(searchLower) ||
    t.email?.toLowerCase().includes(searchLower) ||
    t.class?.toLowerCase().includes(searchLower) ||
    t.section?.toLowerCase().includes(searchLower)
  );
});

const availableTeachers = computed(() => {
  const assignedIds = classTeachers.value.map(ct => ct.id);
...
const uniqueClassesCount = computed(() => {
  return new Set(classTeachers.value.map(ct => ct.class)).size;
});

const totalStudentsCount = computed(() => {
  return classTeachers.value.reduce((sum, ct) => sum + (ct.student_count || 0), 0);
});
...
const fetchTeachers = async () => {
  try {
    const response = await api.get('/admin/teachers?limit=1000');
    if (response.data.success) {
...
const fetchSections = async () => {
  try {
    const response = await api.get('/admin/sections?limit=1000');
    if (response.data.success) {
...
const assignClassTeacher = async () => {
  if (!form.value.teacher_id || !form.value.class_id || !form.value.section_id) {
  ...
  try {
    const response = await api.post('/admin/assign-class-teacher', form.value);
    if (response.data.success) {
      toast.success(response.data.message || 'Class teacher assigned successfully');
      closeModal();
      await fetchClassTeachers();
      await fetchTeachers(); // Refresh available teachers list
    } else {
...
const removeClassTeacher = async (teacherId: number) => {
  if (!confirm('Are you sure you want to remove this teacher as class teacher?')) return;
  
  try {
    const response = await api.delete(`/admin/class-teachers/${teacherId}`);
    if (response.data.success) {
...
const openAssignModal = () => {
  form.value = { teacher_id: '', class_id: '', section_id: '' };
