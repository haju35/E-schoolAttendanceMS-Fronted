...
const fetchChildren = async () => {
  try {
    const response = await familyApi.get('/api/parent/children', {
      headers: { Authorization: `Bearer ${user.token}` }
    })
    children.value = response.data.children
    
    // Check if route has child_id parameter
...
const loadAttendance = async () => {
  ...
  try {
    const response = await familyApi.get(`/api/parent/children/${selectedChild.value.id}/attendance`, {
